...
class VerifyRequest(BaseModel):
    """Request model for pharmacist license verification"""

    license_number: str = Field(
        ...,
        min_length=10,
        max_length=20,
        description="Pharmacist license number (e.g., P2025D00463)",
        examples=["P2025D00463"],
    )
...
class PharmacistData(BaseModel):
    """Complete pharmacist verification data"""

    full_name: Optional[str] = Field(None, description="Pharmacist's full name (formatted)")
    name: Optional[str] = Field(None, description="Pharmacist's name from search")
    ...
    photo_url: Optional[str] = Field(None, description="URL to pharmacist's photo")
    verified_at: str = Field(..., description="Timestamp of verification in ISO format")


class VerifyResponse(BaseModel):
    """Response model for pharmacist license verification"""
    ...
    data: Optional[PharmacistData] = Field(
        None, description="Pharmacist data (null if verification failed)"
    )
